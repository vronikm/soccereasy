<?php
	...
	class facturasController extends mainModel{
			...
			foreach($datos as $rows){				
				...
				$tabla.='
					<tr '.$class.'>
						...
						<td>
							<a href="'.APP_URL.'facturasNew/'.$rows['alumno_id'].'/" class="btn float-right '.$botonpago.' btn-sm" target="_blank">Registrar factura</a>	
						</td>
					</tr>';	
			}
			return $tabla;			
		...
		public function listarPagosFactura($alumnoid, $fecha_inicio, $fecha_fin){ //29052024			
			...
			$datos = $datos->fetchAll();
			foreach($datos as $rows){		
				$checkbox_id = "pagoCheckbox".$rows['pago_id'];	// id único por pago			
				$tabla.='
					<tr>
						...
						<td>
							<div class="custom-control custom-checkbox">
								<input class="custom-control-input chk-pago" 
									...
									value="'.$rows['pago_id'].'"
									data-codigo="'.$rows['codigo'].'"
									data-fecha="'.$rows['pago_fecharegistro'].'"
									data-detalle="Pago '.$rows['detalle'].' del alumno '.$rows['alumno'].'"
									data-valor="'.$rows['pago_valor'].'"
									data-tarifa="0">
		...
		public function actualizarRepresentanteFactura(){
			...
			$representante=$this->ejecutarConsulta("SELECT repre_id FROM alumno_representante WHERE repre_id='$repreid'");
			...
			if (!$this->validarCedula($repre_identificacion)) {
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Error",
			...
			if($this->actualizarDatos("alumno_representante",$representante_reg,$condicion)){
				$alerta=[
					"tipo"=>"recargar",
					"titulo"=>"Informacón actualizada",
					"texto"=>"El representante se actualizó correctamente",
					"icono"=>"success"
				];					
			}else{
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Información no actualizada",
